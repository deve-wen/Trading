...
//|     空头: 反弹阳线→被阴线吞没→收盘开空                            |
//|  3. 无止损, 固定金额止盈(默认$6.6)                                 |
//|  4. 趋势反转时2x倍投(也需吞没形态确认), 同组总利润达标全部平仓    |
//+------------------------------------------------------------------+
...
#property description "③ 无止损,固定金额止盈(默认$6.6),同组全部平仓"
...
input group   "=== ★ 开仓与倍投 ==="
input double  InpTPAmount        = 6.6;             // 同组总利润达标值(账户货币)
...
input bool    InpUseStopLoss     = true;            // 启用单笔止损(亏$N平仓)
input double  InpSL_Dollars      = 10.0;            // 单笔最大亏损(账户货币)
input bool    InpUseTrailing     = true;            // 启用追踪止损
input double  InpTrailActivate   = 2.0;             // 追踪激活利润(账户货币)
input double  InpTrailDistance   = 1.0;             // 追踪回撤距离(账户货币)
...
int OnInit()
{
   ...
   //--- 打印启动信息
   Print("✅ EMA55_吞没_倍投 EA v2.10 启动");
   Print("   交易品种: ", _Symbol, " | 周期: ", EnumToString(_Period));
   Print("   EMA:", InpEMAPeriod, " | 止盈:$", InpTPAmount, " | 最高倍投:", InpMaxDoubles, "次");
   if(InpUseStopLoss)
      Print("   止损: 开启 | 每单亏损-$", InpSL_Dollars, "平仓");
   else
      Print("   止损: 关闭");
   if(InpUseTrailing)
      Print("   追踪: 开启 | 利润>$", InpTrailActivate, "激活, 回撤>$", InpTrailDistance, "平仓");
   else
      Print("   追踪: 关闭");
...
bool IsBullishEngulfing(int idx)
{
   ...
   // 阳线实体完全覆盖阴线实体:
   // 阴线实体范围 = close1~open1 (close1 < open1)
   // 阳线实体范围 = open2~close2 (open2 < close2)
   // 覆盖条件: open2 <= close1 && close2 >= open1
   return(open2 <= close1 && close2 >= open1);
}
...
bool IsBearishEngulfing(int idx)
{
   ...
   // 阴线实体完全覆盖阳线实体:
   // 阳线实体范围 = open1~close1 (open1 < close1)
   // 阴线实体范围 = close2~open2 (close2 < open2)
   // 覆盖条件: open2 >= close1 && close2 <= open1
   return(open2 >= close1 && close2 <= open1);
}
...
void ProcessPositionSLTrailing()
{
   ...
   //--- 跨Tick追踪状态(用static数组持久化)
   static ulong trailTickets[10];
   static double trailBestPrices[10];
   static bool trailActivated[10];
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      //============================================================
      // (A) 固定美元止损: 单笔亏损 >= InpSL_Dollars → 平仓
      //============================================================
      if(InpUseStopLoss && profit <= -InpSL_Dollars)
      {
      ...
      {
         if(trailTickets[t] == ticket)
         {
            found = true;

            // 利润 >= 激活阈值 → 激活追踪
            if(!trailActivated[t] && profit >= InpTrailActivate)
            {
               trailActivated[t] = true;
               trailBestPrices[t] = curPrice;
            ...
            {
               // 更新最优价格
               if(posType == POSITION_TYPE_BUY && curPrice > trailBestPrices[t])
                  trailBestPrices[t] = curPrice;
               else if(posType == POSITION_TYPE_SELL && curPrice < trailBestPrices[t])
                  trailBestPrices[t] = curPrice;

               // 换算美元→点数: profitPerPoint = volume * tickValue / tickSize
               double profitPerPoint = volume * tickValue / tickSize;
               if(profitPerPoint <= 0) continue;

               double trailPts  = InpTrailDistance / profitPerPoint;
               double slPrice   = 0;
               double currSL    = PositionGetDouble(POSITION_SL);

               if(posType == POSITION_TYPE_BUY)
               {
                  slPrice = trailBestPrices[t] - trailPts * _Point;
            ...
            if(g_trailCount < 10)
            {
               ...
               g_trails[g_trailCount].activated = trailActivated[t];
               g_trails[g_trailCount].bestPrice = trailBestPrices[t];
               g_trailCount++;
            }
            break;
         }
      ...
      if(!found && trailCount < 10)
      {
         trailTickets[trailCount]    = ticket;
         trailActivated[trailCount]  = false;
         trailBestPrices[trailCount] = curPrice;
         if(g_trailCount < 10)
         {
            g_trails[g_trailCount].ticket    = ticket;
            g_trails[g_trailCount].direction = (posType == POSITION_TYPE_BUY) ? 1 : -1;
...
void OnTick()
{
   ...
   if(g_group.active)
   {
      ...
      if(totalProfit >= InpTPAmount)
      {
         Print("✅ 同组总利润 $", DoubleToString(totalProfit, 2), " ≥ $", InpTPAmount, " 全部平仓!");
      ...
      //--- 7b) 检查倍投
      //    条件: 趋势反转 + 新方向吞没形态确认 + 每Bar最多1次
      if(g_group.currentLevel < InpMaxDoubles && completedBarTime != g_lastDoubleBar)
      {
         // 倍投方向 = 与最后一单相反
         int lastDir = g_group.dirs[g_group.currentLevel];
         int desiredDir = -lastDir;

         // 当前吞没信号方向必须与desiredDir一致

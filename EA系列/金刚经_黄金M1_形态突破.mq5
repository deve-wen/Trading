...
#property description "③ M顶三模式: 镜像; 平台突破只做顺趋势方向"
...
input bool   InpUsePlatform      = true;     // 启用平台突破(只做顺趋势方向)
...
int OnInit()
{
   ...
   Print("   出场: 每笔强制SL(形态极值) + TP(2×SL, 盈亏比2:1)");
...
int DetectWBottom(double &slTarget)
{
   ...
   if(InpUseWBottom)
   {
      ...
      if(idxN > 0)
      {
         double N = iHigh(_Symbol, InpPatternTF, idxN);
         // 颈线高于两个底
         if(N > L0 + InpMinWavePts * point && N > L1 + InpMinWavePts * point)
         {
            // 收盘突破颈线 + 缓冲
            if(close1 > N + InpNeckBufferPts * point)
            {
               double lowest = MathMin(L0, L1);      // W底最低点
               slTarget = lowest - InpSLBufferPts * point;   // 止损=W底下方20点
               return 1;
            }
         }
      }
...
int DetectMTop(double &slTarget)
{
   ...
   if(InpUseMTop)
   {
      ...
      if(idxN > 0)
      {
         double N = iLow(_Symbol, InpPatternTF, idxN);
         // 颈线低于两个顶
         if(N < H0 - InpMinWavePts * point && N < H1 - InpMinWavePts * point)
         {
            // 收盘跌破颈线 - 缓冲
            if(close1 < N - InpNeckBufferPts * point)
            {
               double highest = MathMax(H0, H1);   // M顶最高点
               slTarget = highest + InpSLBufferPts * point;   // 止损=M顶上方20点
               return 1;
            }
         }
      }
...
//+------------------------------------------------------------------+
//| 平台突破检测 (金刚经: 平台越长区间越窄越好)                       |
//| 多头: 收盘突破平台上沿 → 做多; 空头: 跌破下沿 → 做空             |
//+------------------------------------------------------------------+
int DetectPlatform()
{
   // ★ v1.01修复: 平台区间只看bar2~barLookback(排除突破K线bar1本身)
   //   旧代码含bar1: 突破K线的高点被计入hi → close1永远无法>hi → 平台永废
   ...
   // 平台区间必须窄 (金刚经: 区间越窄越好)
   if(range > InpPlatMaxRangePts * point) return 0;
...
void CheckEntry()
{
   ...
   // ★ 强制止盈: 盈亏比 2:1 → TP = 2 × SL区间 (与开关无关, 每笔必带)
   double tpPoints = 2.0 * slPoints;
   ...
   if(g_Common.OpenPosition(signal, entryPrice, slPoints, tpPoints))
   {
      ...
      // 精确修正形态SL/TP (金刚经: 止损设在形态极值绝对价格, 止盈2:1)
                        ...
                        const MqlTradeResult& result)
{
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      ...
      {
         // 无需额外处理, 持仓数由CountPositions()实时判断
      }
   }
}
//+------------------------------------------------------------------+

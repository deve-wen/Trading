...
//|  入场: 形态收盘确认 → 下根K线开盘市价入场                         |
//|  止损: 形态极值 - 20点 (绝对价格, 每笔必带)                       |
...
#property description "① 趋势过滤+形态入场+W底/M顶三模式(标准/激进①/激进②独立开关)"
#property description "② 每笔强制SL+TP(盈亏比可调); 风控: 止损≥1500点拒开; 追踪A/B/C三选一"
#property description "③ v1.10诊断(自动时区+日志); v1.11有效突破=盘中突破颈线+收盘确认"
#property description "④ v1.12颈线≥200点+整体极值止损; v1.13平仓冷却+盈亏比可调"
#property description "⑤ v1.14分批止盈(1/2/3倍SL分批平+比例可调); v1.15先触发第1档才能开第2仓"
#property description "⑥ v1.16 GV持久化; v1.17/1.19保护门独立于参数; v1.20防历史回放永不入场"
#property description "⑦ v1.21顺势关键位突破; v1.22防追高三重约束(颈线年龄/突破距离/首次突破)"
#property description "⑧ v1.23收盘强平+禁开仓(不持仓过夜): 结束前2分钟平全部+结束前30分钟禁开仓"
...
input bool   InpUsePlatform      = true;     // ★v1.21 启用顺势关键位突破(金叉多头突破前高做多,死叉空头跌破前低做空)
...
input int    InpNeckHeightMinPts = 200;      // ★v1.12 颈线距两底/两顶最小高度(点, 标准模式要求≥此值)
...
input group "=== ★ v1.22 防追高(刚突破约束, 拒绝\"早就走完形态\"的开仓) ==="
input bool   InpRequireFirstBreak = true;    // ★要求bar1是首次突破(bar2最高<颈线才开仓)
input int    InpMaxBreakoutDistPts = 100;    // ★bar1收盘距离颈线最大点数(超出视为追高, 拒绝开仓)
input int    InpMaxNeckAgeBars    = 30;      // ★颈线距今最大K线数(颈线太老不构成有效W底/M顶)

input group "=== ★ 出场参数 ==="
input int    InpSLBufferPts      = 20;       // 形态止损缓冲(点)
input int    InpMaxPositions     = 2;        // ★v1.14 最大同时持仓数(2=分批止盈平部分后可开新仓)
input double InpRewardRiskRatio  = 2.0;      // 盈亏比(TP/SL倍数): 2=2:1, 3=3:1, 5=5:1等

input group "=== ★ v1.14 分批止盈(独立开关, 每档比例可调) ==="
...
input group "=== ★ v1.13 平仓冷却(同根K线禁止再开仓) ==="
...
input group "=== ★ v1.23 收盘强平 + 禁开仓(不持仓过夜) ==="
input bool   InpUseEndCloseAll        = true;    // ★启用收盘强平(交易结束前强制平仓所有持仓, 不论盈亏)
input int    InpCloseAllMinsBeforeEnd = 2;       // ★强平提前量(分钟) — 距交易结束≤N分钟 → 强平全部持仓
input int    InpNoEntryMinsBeforeEnd  = 30;      // ★禁开仓提前量(分钟) — 距交易结束≤N分钟 → 禁止开新仓
...
int      g_magic            = 20260804;
datetime g_lastCloseBarTime = 0;   // ★v1.13 上次平仓所在K线时间(同根K线冷却用)
datetime g_endCloseDate     = 0;   // ★v1.23 收盘强平日期(同一日只强平一次, 防OnTick刷屏)

//+------------------------------------------------------------------+
//| ★v1.14 分批止盈状态槽位 (每持仓记录: 止损空间 + 已执行档位)        |
//+------------------------------------------------------------------+
struct TierState
{
...
bool      g_anyTier1Triggered = false;  // ★v1.15 是否有任意持仓走过第1档分批止盈(触发后才允许开第2仓)
string    g_gvTier1Fired = "";          // ★v1.16 持久化键名(用magic区分EA), EA重启状态保留

//+------------------------------------------------------------------+
//| ★v1.16 持久化辅助: 跨EA重启保留分批止盈第1档触发状态              |
//+------------------------------------------------------------------+
void LoadTier1FiredFlag()    // OnInit 时调用: 从MT5全局变量读取持久化状态
{
   g_anyTier1Triggered = (GlobalVariableCheck(g_gvTier1Fired) && GlobalVariableGet(g_gvTier1Fired) > 0.5);
}
void SaveTier1FiredFlag()    // 触发第1档时调用: 同步写入MT5全局变量(EA重启仍有效)
{
...
{
   g_magic = 20260804;
   g_gvTier1Fired = "JGJ_T1F_" + IntegerToString(g_magic);   // ★v1.16 用magic区分持久化键
   g_lastCloseBarTime = 0;
   ZeroMemory(g_tierSlots);
   LoadTier1FiredFlag();  // ★v1.16 从MT5全局变量恢复触发状态(跨EA重启)
   ...
   if(InpMaxBreakoutDistPts < 10) { Print("❌ bar1收盘距离颈线最大点数必须>=10"); return(INIT_PARAMETERS_INCORRECT); }
   if(InpMaxNeckAgeBars < 5)     { Print("❌ 颈线最大年龄K线数必须>=5"); return(INIT_PARAMETERS_INCORRECT); }

   //--- 通用模块初始化
   if(!g_Common.Init(g_magic, "JGJ_M1"))
   {
      Print("❌ 通用模块初始化失败");
      return(INIT_FAILED);
   }
   ...
   Print("   v1.23收盘强平: ", (InpUseEndCloseAll ? "开启" : "关闭"),
         " (结束前≤", InpCloseAllMinsBeforeEnd, "分钟平全部)",
         " | 禁开仓: 结束前≤", InpNoEntryMinsBeforeEnd, "分钟禁止开新仓");
   if(InpUseTrailingA)
   ...
   Print("   ★v1.13 平仓冷却: ", (InpUseCloseCooldown ? "启用" : "禁用"),
         " 平仓后", InpCloseCooldownBars, "根K线内禁止再开仓 | 盈亏比=", DoubleToString(InpRewardRiskRatio, 1), ":1");
   Print("   ★v1.14 分批止盈: ", (InpUseTieredTP ? "启用" : "禁用"),
   ...
   Print("   ★v1.16 分批保护: InpUseTieredTP=", (InpUseTieredTP ? "true" : "false"),
         " → 第1档触发状态=", (g_anyTier1Triggered ? "已触发(允许开第2仓)" : "未触发(第2仓将拦截)"),
         " | 持久化键=", g_gvTier1Fired,
         " | GV当前值=", (GlobalVariableCheck(g_gvTier1Fired) ? DoubleToString(GlobalVariableGet(g_gvTier1Fired), 0) : "不存在"));
...
void OnTick()
{
   ...
   // 2. ★v1.14 分批止盈: 利润达1/2/3倍止损空间时按比例分批平仓
   CheckTieredTP();

   // 3. ★v1.23 收盘强平: 距结束≤InpCloseAllMinsBeforeEnd → 平全部(防持仓过夜)
   EndCloseAllIfNeeded();

   // 4. 新M1 K线检测(用iTime时间戳, 稳定)
...
void OnTimer()
{
   ...
   CheckTieredTP();     // ★v1.14 非交易时间也要执行分批止盈
   EndCloseAllIfNeeded(); // ★v1.23 非交易时间也要检查(虽然结束时段外不会触发)
}
      ...
      if(g_tierSlots[slot].tierReached == 0 && profitPts >= InpTier1ProfitMult * slPoints)
      {
         ...
         if(closeLots > 0 && closeLots < volume)
         {
            if(g_trade.PositionClosePartial(ticket, closeLots))
            {
               ...
               SaveTier1FiredFlag();   // ★v1.16 同步持久化触发状态(EA重启后仍生效)
...
int DetectWBottom(double &slTarget)
{
   ...
   // ========== 模式1: 标准 (突破颈线) ==========
   // ★ v1.11优化: 有效突破 = 盘中价格突破颈线 + 收盘价依然在颈线之上(收盘确认)
   //   止损 = 第二个底部L1下方20点(不再用整个W底最低点, 止损更小更贴形态)
   // ★ v1.22 防追高: 三重"刚突破"约束
   //   ① 颈线年龄 ≤ InpMaxNeckAgeBars(防止颈线N是几十根K线前的高点, 那时形态早已完成)
   //   ② bar1收盘距离颈线 ≤ InpMaxBreakoutDistPts(防止"已突破很久才入场"导致的止损过大)
   //   ③ 若 InpRequireFirstBreak=true: bar2最高 < 颈线(确保bar1是首次突破, 不是"颈线之上震荡")
   if(InpUseWBottom)
   {
      ...
      {
         double N = iHigh(_Symbol, InpPatternTF, idxN);
         // ★v1.12 颈线高于两底 ≥ InpNeckHeightMinPts(默认200点) — 形态高度不够不构成有效W底
         if(N > L0 + InpNeckHeightMinPts * point && N > L1 + InpNeckHeightMinPts * point)
         {
            // ★v1.22 ① 颈线年龄约束(防止"颈线太老"的假突破)
            if(idxN > InpMaxNeckAgeBars)
               return 0;

            // ★v1.11 有效突破: 该K线盘中最高价突破颈线位 + 收盘价依然收在颈线之上
            if(high1 > N && close1 > N)
            {
               // ★v1.22 ② 突破距离约束: bar1收盘距离颈线 ≤ 阈值(防止追高)
               if(close1 - N > InpMaxBreakoutDistPts * point)
                  return 0;

               // ★v1.22 ③ 首次突破约束(可选): bar2 最高 < 颈线, 保证 bar1 是首次突破
               if(InpRequireFirstBreak)
               {
                  double high2 = iHigh(_Symbol, InpPatternTF, 2);
                  if(high2 >= N)
                     return 0;   // bar2 已突破, 不是首次
               }

               // ★v1.12 止损: 整个W底形态最低点下方20点
...
int DetectMTop(double &slTarget)
{
   ...
   // ========== 模式1: 标准 (跌破颈线) ==========
   // ★ v1.11优化: 有效突破 = 盘中价格跌破颈线 + 收盘价依然在颈线之下(收盘确认)
   //   止损 = 第二个顶部H1上方20点(不再用整个M顶最高点, 止损更小更贴形态)
   // ★ v1.22 防追空: 三重"刚突破"约束(镜像W底)
   if(InpUseMTop)
   {
      ...
      {
         double N = iLow(_Symbol, InpPatternTF, idxN);
         // ★v1.12 颈线低于两顶 ≥ InpNeckHeightMinPts(默认200点) — 形态深度不够不构成有效M顶
         if(N < H0 - InpNeckHeightMinPts * point && N < H1 - InpNeckHeightMinPts * point)
         {
            ...
            //   (排除假突破: 盘中刺穿但收盘收回到颈线上方 = 无效)
            if(low1 < N && close1 < N)
            {
               // ★v1.22 ② 突破距离约束: bar1收盘距离颈线 ≤ 阈值(防止追空)
               if(N - close1 > InpMaxBreakoutDistPts * point)
                  return 0;

               // ★v1.22 ③ 首次突破约束(可选): bar2 最低 > 颈线, 保证 bar1 是首次跌破
               if(InpRequireFirstBreak)
               {
                  double low2 = iLow(_Symbol, InpPatternTF, 2);
                  if(low2 <= N)
                     return 0;   // bar2 已跌破, 不是首次
               }

               // ★v1.12 止损: 整个M顶形态最高点上方20点
...
void CheckEntry()
{
   ...
   // ★v1.23 禁开仓: 距交易结束≤InpNoEntryMinsBeforeEnd分钟 → 不开新仓(防持仓过夜)
   if(IsEntryForbiddenByEnd())
   {
      if(InpDiagnosticLog)
         Print("🩺 诊断: 禁开仓时段(距交易结束 ", MinutesToSessionEnd(), " 分钟)");
      return;
   }

   // 2. ★v1.19 持仓上限彻底修复(独立于所有参数):
   //   实际允许持仓数 = g_anyTier1Triggered ? InpMaxPositions : 1
   //   - 完全去掉 InpUseTieredTP 条件, 保护门永远生效
   //   - 开仓成功后立即重置 g_anyTier1Triggered=false (每个新仓位独立计算)
   ...
   // 7. 执行开仓 (v1.04: 强制SL+TP, TP=2×SL区间)
   ...
   // ★v1.13 盈亏比可调: TP = 入场 ± InpRewardRiskRatio × SL区间 (默认2:1, 可3:1/5:1等)
   // ★v1.14 启用分批止盈时: TP放远到第3档倍数位置, 由分批止盈主导出场(避免固定TP抢先全平)
   ...
   if(g_Common.OpenPosition(signal, entryPrice, slPoints, tpPoints))
   {
      ...
      if(ticket != 0)
      {
         ...
         // ★v1.14 注册分批止盈槽位 (记录该单止损空间, 作为1/2/3倍利润基准)
         RegisterTier(ticket, slPoints);
         // ★v1.19 开仓成功后重置触发状态: 每个新仓位需要独立走到1×SL才能开下一仓
         //   (避免历史记录残留导致"第2仓立即开"的漏洞)
...
void SetExactSLTP(ulong ticket, double slTarget, double tpTarget)
{
   ...
   // ★v1.13 强制盈亏比止盈: 若外部未提供tpTarget, 按入场价±InpRewardRiskRatio×SL距离计算
   if(tpTarget <= 0)
   {
...
//+------------------------------------------------------------------+
//| ★v1.23 距交易结束还有多少分钟(>0=在交易时段内且剩余分钟数)           |
//| 返回 -1 表示不在交易时段内(由IsTradingAllowed统一拦截)              |
//+------------------------------------------------------------------+
int MinutesToSessionEnd()
{
   ...
   if(endMinutes < startMinutes)
   {
      // 段1: start ~ 24:00, 段2: 00:00 ~ end
      if(currentMinutes >= startMinutes)
         return (24 * 60 - currentMinutes) + endMinutes;     // 段1内: 距段1结束=距次日段2结束
      else if(currentMinutes <= endMinutes)
         return endMinutes - currentMinutes;                  // 段2内: 距结束直接差
      else
         return -1;                                           // 05:00~07:30 交易时段外
   }
   else
   {
      if(currentMinutes >= startMinutes && currentMinutes <= endMinutes)
...
//+------------------------------------------------------------------+
//| ★v1.23 禁开仓检查: 距交易结束≤InpNoEntryMinsBeforeEnd分钟 → 禁开仓 |
//+------------------------------------------------------------------+
bool IsEntryForbiddenByEnd()
{
   int minsLeft = MinutesToSessionEnd();
   if(minsLeft < 0) return false;     // 交易时段外由 IsTradingAllowed 拦截
   return (minsLeft <= InpNoEntryMinsBeforeEnd);
}

//+------------------------------------------------------------------+
//| ★v1.23 收盘强平: 距交易结束≤InpCloseAllMinsBeforeEnd分钟 → 平全部   |
//| 不论盈亏, 防止持仓过夜(周末跳空/周一开盘跳空风险)                 |
//+------------------------------------------------------------------+
void EndCloseAllIfNeeded()
{
   if(!InpUseEndCloseAll) return;

   int minsLeft = MinutesToSessionEnd();
   if(minsLeft < 0) return;                                  // 交易时段外
   if(minsLeft > InpCloseAllMinsBeforeEnd) return;           // 还不到强平时段

   // 防同一天内重复调用(OnTick 每秒多次), 当天 0 点作日期标记
   datetime beijingToday = (BeijingTimeNow() / 86400) * 86400;
   if(g_endCloseDate == beijingToday) return;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(g_trade.PositionClose(ticket))
         ...
         PrintFormat("⚠️ 收盘强平失败 ticket=%I64u retcode=%u (%s)",
                     ticket, g_trade.ResultRetcode(), g_trade.ResultRetcodeDescription());
   }

   if(closedCount > 0)
   {
      Print("⏰ 收盘强平: 已平 ", closedCount, " 单 (距交易结束 ", minsLeft,
            " 分钟, 不论盈亏) — ", TimeToString(beijingToday, TIME_DATE));
      g_endCloseDate = beijingToday;                          // 标记今天已强平
   }
}
...
{
   // ★v1.20 平仓冷却: 检测平仓交易(DEAL_ENTRY_OUT), 记录平仓所在K线时间
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      if(trans.deal != 0 && HistoryDealSelect(trans.deal))
      {
         ...
         {
            // ★v1.20修复: 记录平仓时当前K线(bar0)时间戳, 而非bar1(已收盘)
            //   这样当bar0变为bar1后 sigBarTime==g_lastCloseBarTime → 正确拦截

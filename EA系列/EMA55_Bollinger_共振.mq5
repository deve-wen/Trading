//+------------------------------------------------------------------+
//|                                        EMA55_Bollinger_共振.mq5 |
//|                                          Senior Developer        |
//|                                                                  |
//|  策略: M5 EMA55趋势判断 + M1 布林带回踩入场                       |
//|                                                                  |
//|  【趋势判断 - M5】                                                |
...
//|                                                                  |
//|  【入场 - M1】                                                    |
//|    多头: M5多头 + 价格回踩布林带下轨 开多                        |
//|    空头: M5空头 + 价格回踩布林带上轨 开空                        |
//|                                                                  |
//|  【出场】                                                         |
//|    止损: 300点(3美金) | 止盈: 600点(6美金)                       |
//|    追踪止损: 200点激活 → 500点保本                                |
//|                                                                  |
//|  【风控】                                                         |
...
#property description "M5 EMA55趋势 + M1 布林带回踩入场 EA"
#property description "M5:连续2K收盘>55EMA=多头,收盘<55EMA=空头"
#property description "M1:多头→价格回踩下轨开多,空头→价格回踩上轨开空"
#property description "止损300点/止盈600点,追踪止损200点激活500点保本"
...
//+------------------------------------------------------------------+
input group   "=== ★ 策略核心参数 ==="
input int     InpMagicNumber     = 20260520;        // 魔法编号
input double  InpStopLoss        = 300;             // 止损(点数)
input double  InpTakeProfit      = 600;             // 止盈(点数)
...
input group   "=== ★ 亏损暂停 ==="
input bool    InpUseLossPause    = true;            // 启用亏损暂停
input int     InpLossPauseSec    = 300;              // 亏损暂停秒数(默认5分钟)
...
int         g_hBB     = INVALID_HANDLE;   // M1 布林带指标句柄
...
//--- M1 布林带状态
double      g_bbUpper    = 0.0;           // 布林带上轨
double      g_bbMiddle   = 0.0;           // 布林带中轨(均线)
double      g_bbLower    = 0.0;           // 布林带下轨
bool        g_bbReady    = false;         // 布林带数据就绪
...
//--- 亏损暂停状态
datetime    g_pauseUntil       = 0;          // 暂停到何时(服务器时间,0=未暂停)
ulong       g_lastAnalyzedDeal = 0;          // 上次已分析的平仓单ticket
bool        g_hadPosition      = false;      // 上一周期是否有持仓(用于检测平仓)
...
int OnInit()
{
   ...
   if(g_hMA55 == INVALID_HANDLE || g_hBB == INVALID_HANDLE)
   {
   ...
   //--- 打印启动信息
   Print("╔══════════════════════════════════════════╗");
   Print("║   M5 EMA55 + M1 布林带 共振EA v1.02      ║");
   Print("╠══════════════════════════════════════════╣");
   ...
   Print("║ BB周期/偏差: ", InpBBPeriod, "/", InpBBDeviation);
   Print("║ 追踪激活:   ", InpTrailActivate, "点");
   Print("║ 追踪保本:   ", InpTrailDistance, "点");
   Print("║ 亏损暂停:   ", (InpUseLossPause ? (string)InpLossPauseSec + "秒(每笔)" : "关"));
   Print("╚══════════════════════════════════════════╝");
...
void UpdateBB()
{
   ...
   //--- 布林带缓冲区索引: 0=BASE(中轨) 1=UPPER(上轨) 2=LOWER(下轨)
   int c1 = CopyBuffer(g_hBB, 0, 1, 1, middle);   // 中轨
   int c2 = CopyBuffer(g_hBB, 1, 1, 1, upper);    // 上轨
   int c3 = CopyBuffer(g_hBB, 2, 1, 1, lower);    // 下轨
   ...
   if(middle[0] <= 0 || upper[0] <= 0 || lower[0] <= 0)
   {
      Print("⚠️ 布林带数据异常,等待更多数据");
      return;
   }
   ...
   {
      g_bbReady = true;
      Print("📐 布林带就绪: 中轨=", DoubleToString(g_bbMiddle, g_dig),
            " 上轨=", DoubleToString(g_bbUpper, g_dig),
            " 下轨=", DoubleToString(g_bbLower, g_dig));
   }
}
...
//|  核心逻辑(★v1.02 简化版★):                                        |
//|  ┌────────────────────────────────────────────────────────┐       |
//|  │ 每笔亏损出场 → 强制暂停 N 秒(N=InpLossPauseSec,默认300) │       |
//|  │ 盈利出场 → 不影响暂停状态(可提前恢复)                   │       |
//|  └────────────────────────────────────────────────────────┘       |
//+------------------------------------------------------------------+
void CheckPositionClose()
{
   ...
   if(g_hadPosition && !hasPos)
   {
      ...
      //--- 未启用暂停则跳过
      if(!InpUseLossPause) return;

      //--- 获取最近24小时的交易历史
      datetime now = TimeCurrent();
      if(!HistorySelect(now - 86400, now)) return;
      ...
      for(int i = total - 1; i >= 0; i--)
      {
         ...
         //--- 只处理平仓单(DEAL_ENTRY_OUT)
         long dealEntry = HistoryDealGetInteger(dealTicket, DEAL_ENTRY);
         if(dealEntry != DEAL_ENTRY_OUT) continue;

         //--- 跳过已分析过的平仓单
         if(dealTicket == g_lastAnalyzedDeal) break;

         g_lastAnalyzedDeal = dealTicket;

         double profit = HistoryDealGetDouble(dealTicket, DEAL_PROFIT);

         //==========================================================
         // ★ 亏损 → 强制暂停 (每笔亏损都触发,不依赖连损计数)
         //==========================================================
         if(profit < 0)
         {
            g_pauseUntil = now + InpLossPauseSec;
            Print("⏸️⏸️⏸️ 亏损出场 → 暂停 ", InpLossPauseSec/60, " 分钟! ",
                  "恢复时间: ", TimeToString(g_pauseUntil), " (服务器时间)");
         }
         //==========================================================
         // 盈利 → 如果之前有暂停则提前结束
         //==========================================================
         else if(profit > 0 && g_pauseUntil > 0)
         {
            Print("✅ 盈利出场,提前结束暂停,恢复正常交易");
            g_pauseUntil = 0;
         }
...
//+------------------------------------------------------------------+
//| 亏损暂停检查                                                     |
//|  返回 true 表示当前处于暂停状态,不应开新仓                         |
//+------------------------------------------------------------------+
bool IsLossPaused()
{
   //--- 未启用暂停
   if(!InpUseLossPause) return false;
   ...
   {
      Print("✅ 亏损暂停已到期,恢复正常交易 (暂停 ", InpLossPauseSec, " 秒)");
      g_pauseUntil = 0;
      return false;
   }

   //--- 仍在暂停中
   int remainSec = (int)(g_pauseUntil - TimeCurrent());
   static int s_lastRemainLog = -1;
   int curRemainMin = remainSec / 60;
   if(curRemainMin != s_lastRemainLog)
   {
      s_lastRemainLog = curRemainMin;
      Print("⏸️ 亏损暂停中,剩余 ", remainSec, " 秒后恢复交易");
   }
   return true;
}
...
void CheckEntry()
{
   ...
   //--- 同一根M1K线只入场一次(防止同一根K线多次触发)
   if(g_entryBlock) return;

   //--- 当前K线入场保护: 如果是新K线刚出现,等待至少3秒再入场(防止数据未刷新)
   static datetime s_lastM1Check = 0;
   if(curM1Bar != s_lastM1Check)
   {
      s_lastM1Check = curM1Bar;
      // 新K线头3秒等待数据稳定
      static datetime s_newBarTime = 0;
      s_newBarTime = TimeCurrent();
   }
   ...
   //============================================================
   // 多头入场: M5多头趋势 + 价格回踩布林带下轨
   //============================================================
   if(g_trendM5 == TREND_BULL)
   {
      ...
      if(ask <= g_bbLower)
      {
         ...
         Print("🔵 多头布林带回踩到位! 触发开多");
         Print("   M5趋势=多头  BB下轨=", DoubleToString(g_bbLower, g_dig));
         Print("   当前Ask=", DoubleToString(ask, g_dig),
               " 下轨距离=", DoubleToString(g_bbLower - ask, g_dig), " USD");
         OpenBuy(lot);
      }
   }

   //============================================================
   // 空头入场: M5空头趋势 + 价格回踩布林带上轨
   //============================================================
   if(g_trendM5 == TREND_BEAR)
   {
      double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      ...
      if(bid >= g_bbUpper)
      {
         ...
         Print("🔴 空头布林带回踩到位! 触发开空");
         Print("   M5趋势=空头  BB上轨=", DoubleToString(g_bbUpper, g_dig));
         Print("   当前Bid=", DoubleToString(bid, g_dig),
               " 上轨距离=", DoubleToString(bid - g_bbUpper, g_dig), " USD");
         OpenSell(lot);
      }
   }
}
...
void OpenBuy(double lot)
{
   ...
   if(m_trade.Buy(lot, _Symbol, ask, sl, tp, "BB-Buy"))
   {
...
void OpenSell(double lot)
{
   ...
   if(m_trade.Sell(lot, _Symbol, bid, sl, tp, "BB-Sell"))
   {

...
//=== ★ v2.04 收盘强平与禁开仓(不持仓过夜) ===
input bool   InpUseEndCloseAll     = true;   // ★v2.04 启用收盘强平(交易结束前N分钟强平全部持仓, 不论盈亏)
input int    InpCloseAllMinsBeforeEnd = 2;   // ★v2.04 强平提前量: 距交易结束≤此分钟数时强平所有持仓(默认2分钟)
input int    InpNoEntryMinsBeforeEnd  = 30;  // ★v2.04 禁开仓提前量: 距交易结束≤此分钟数时禁止开新仓(默认30分钟)
...
//+------------------------------------------------------------------+
//| 距交易结束还有多少分钟 ★v2.04新增                                |
//| 返回: ≥0 = 距离结束分钟数; <0 = 已过结束时间(跨日场景下取下一轮)  |
//| 逻辑: 计算当前北京时间到"下一个交易日结束点"的分钟差              |
//|   例: 结束=次日03:30 → 今天03:30已过则取明天03:30                |
//+------------------------------------------------------------------+
...
bool CCommonTradingModule::IsTradingAllowed()
{
   ...
   // ★v2.04 交易结束前 N 分钟内禁止开新仓(不持仓过夜)
   if(InpNoEntryMinsBeforeEnd > 0)
   {
      int minsToEnd = MinutesToSessionEnd();
      if(minsToEnd <= InpNoEntryMinsBeforeEnd)
         return false;
   }
...
int OnInit()
{
   ...
   if(InpUseEndCloseAll && InpNoEntryMinsBeforeEnd <= InpCloseAllMinsBeforeEnd)
      Print("⚠️ 禁开仓提前量(", InpNoEntryMinsBeforeEnd, "分钟) ≤ 强平提前量(", InpCloseAllMinsBeforeEnd,
   ...
   Print("   ★v2.04 不持仓过夜: 收盘强平=", (InpUseEndCloseAll ? "开" : "关"),
...
void OnTick()
{
   ...
   // 1.5 ★v2.04 收盘强平: 距交易结束≤N分钟 → 强平全部持仓(不持仓过夜, 不论盈亏)
...
int DetectPullbackEntry(double &slTarget, string &name)
{
   ...
   if(isBull)
   {
      ...
      if(InpUse2BStructure)
      {
         ...
         int idxHPrev = FindSwingHigh(idxH + PB_STRENGTH + 1, InpPullbackLookback, PB_STRENGTH);
         if(idxHPrev <= 0) return 0;
         double HPrev = iHigh(_Symbol, InpPatternTF, idxHPrev);
         if(H <= HPrev) return 0;   // 高点未升高 → 非HH结构

         // 低点升高: L0 之前更早的摆动低点 < L0 (前低低于当前波段低点)
         int idxLPrev = FindSwingLow(idxL0 + PB_STRENGTH + 1, InpPullbackLookback, PB_STRENGTH);
         if(idxLPrev <= 0) return 0;
         double LPrev = iLow(_Symbol, InpPatternTF, idxLPrev);
         if(L0 <= LPrev) return 0;  // 低点未升高 → 非HL结构
      }
   ...
   else // 空头镜像
   {
      ...
      if(InpUse2BStructure)
      {
         ...
         int idxLPrev = FindSwingLow(idxL + PB_STRENGTH + 1, InpPullbackLookback, PB_STRENGTH);
         if(idxLPrev <= 0) return 0;
         double LPrev = iLow(_Symbol, InpPatternTF, idxLPrev);
         if(L >= LPrev) return 0;   // 低点未降低 → 非LL结构

         // 高点降低: H0 之前更早的摆动高点 > H0 (前高高于当前波段高点)
         int idxHPrev = FindSwingHigh(idxH0 + PB_STRENGTH + 1, InpPullbackLookback, PB_STRENGTH);
         if(idxHPrev <= 0) return 0;
         double HPrev = iHigh(_Symbol, InpPatternTF, idxHPrev);
         if(H0 >= HPrev) return 0;  // 高点未降低 → 非LH结构
      }
...
//+------------------------------------------------------------------+
//| ★v2.04 收盘强平(不持仓过夜)                                      |
//| 距交易结束 ≤ InpCloseAllMinsBeforeEnd 分钟时: 强平本EA全部持仓,   |
//|   不论盈亏均平仓。                                                |
//| 调用点: OnTick / OnTimer (定时器兜底, 无tick也触发)               |
//| 防重复: g_endCloseDate 记录已强平日期(00:00归一化), 同一交易日只  |
//|   执行一次, 避免每个tick重复平仓/重复打印                          |
//+------------------------------------------------------------------+
void CloseAllPositionsAtEnd()
{
   ...
   // 强平本EA全部持仓(不论盈亏)
   int  closedCount = 0;
   bool allClosed   = true;   // 是否全部成功(失败则下个tick重试)
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong ticket = PositionGetTicket(i);
      if(ticket == 0) continue;
      ...
      {
         closedCount++;
         Print("🕛 收盘强平: ticket=", ticket,
               " ", (ptype == POSITION_TYPE_BUY ? "BUY" : "SELL"),
               " 手数=", DoubleToString(volume, 2),
               " 距结束", minsToEnd, "分钟 | 不持仓过夜");
      }
      else
      {
         allClosed = false;
         Print("⛔ 收盘强平失败! ticket=", ticket,
               " retcode=", g_trade.ResultRetcode(),
               " 距结束", minsToEnd, "分钟, 下个tick重试");
      }
   }

   if(closedCount > 0)
      Print("✅ 收盘强平完成: 共平 ", closedCount, " 笔, 全部离场, 不持仓过夜");

   // 全部成功(或本无持仓)才记录日期, 避免失败后本日不再重试
   if(allClosed)
...
void CheckEntry()
{
   ...
   if(!g_Common.IsTradingAllowed())
   {
      if(InpDiagnosticLog)
      {
         ...
         else if(InpNoEntryMinsBeforeEnd > 0 && MinutesToSessionEnd() <= InpNoEntryMinsBeforeEnd)
            reason = "距交易结束≤" + IntegerToString(InpNoEntryMinsBeforeEnd) + "分钟(禁开仓)";
